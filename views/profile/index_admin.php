...
<div class="box">
    ...
    <div class="box-body">
        <p class="pull-left">
            <?= Html::button( ' ' .$this->title, [
                        'value' => Url::to(['profile/create']),
                        'title' => $this->title,
...
<?php foreach ($models as $profile): ?>
                    <tr>
                        ...
                        <td  ><a class="activity-view-link" href="#" data-id="<?= $profile->id ?>"><?= $profile->fullname ?></a><?php
                            ...
                            ?>  </td>
                        <td class="text-center" ><?= $profile->user['username']; ?>
                        </td>
                        <td>                                
                            <a class="btn btn-info btn-xs" href="?r=profile/resetpass&id=<?= $profile->id ?>" data-id="<?= $profile->id ?>" onclick="return confirm('คุณแน่ใจนะ')" >Reset Password</a>
                            ...
                            <?php if ($profile->user['status'] == 0) { ?>
                                ...
                                <?php
                                echo Html::a('ลบ', ['profile/delete', 'id' => $profile->id], [
                                    'class' => 'glyphicon glyphicon-remove pull-right btn btn-danger btn-xs',
                                    'data' => [
